...
volatile struct tfwlist_header_t *global_timer_requestes_queue = NULL;
...
void kernel()
{
    ...
    global_timer_requestes_queue = __pcb_queue_base__ + 48 * 16;
...
void wakeup_service()
{
    ...
    if (temp_task != NULL)
        for (u64_t i = 0; i < 128; i++)
        {
            ...
            volatile struct timer_request_t *req = tfw_find(global_timer_requestes_queue, temp_task->id);
            ...
            if (req > 1) // if was correct and valid.
            {
                ...
                {
                    fw_rm(global_timer_requestes_queue, tfw_idx(global_timer_requestes_queue, temp_task->id)); // remove from timer requests queue.
