...
void kernel_serial_service()
{
    ...
    while (1)
    {
        ...
        if (message.content_pt1 & 0x1)                                                                                                                    // if first bit set, its write, else read.
            mxwrite(mailbox, serial_put((char *)message.content_pt2, ((u64_t)message.content_pt1) & (0xFFFFFFFF << 1)), 0, true, message.author_task_id); // response output code of mini-uart write.
        else
