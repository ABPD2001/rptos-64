...
typedef struct ipcmailbox_t // 56 Bytes.
{
    ...
    u64_t *blacklist_tasks_id; // this list cant access (if isnt NULL).
    u64_t *whitelist_tasks_id; // this list can access only (if isnt NULL).
    u64_t accessibility;       // 0: write access, 1: read access, 2: management, 3-63: reserved.
    u32_t maximum_length;
    u32_t metadata; // 0: universal, 1: oneline, 2: dual, 3-4: (0 -> empty, 1 -> writing, 2 -> fill, 3 -> reading), 5-31: reserved.
};
...
typedef struct ipcmailbox_segment_t // 56 Bytes.
{
    mutex_t access_mutex; // a mutex, where task wants to write/read.
    u64_t mailbox_id;
    u32_t mailbox_type;                  // 0: universal, 1: oneline, 2: dual, 3: raw.
    u32_t status;                        // 0: empty, 1: filling, 2: full, 3: reading. (1/3 means is reserved.) this is just for general information about mutex.
